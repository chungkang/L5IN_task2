...
if len(filtered_matched_lists) > 0:
    ...
    keypoints = np.float32([kp2[m.trainIdx].pt for m in filtered_matched_lists])
    ...
    for label in unique_labels:
        ...
        src_pts = np.float32([kp1[m.queryIdx].pt for m, lbl in zip(filtered_matched_lists, labels) if lbl == label]).reshape(-1, 1, 2)
...
result_img = cv2.drawMatchesKnn(templateImage, kp1, background_img, kp2, matches_list, None, flags=2)
...
img3 = cv2.drawMatchesKnn(templateImage,kp1,background_img,kp2,matches,None,**draw_params)
